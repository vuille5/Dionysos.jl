...
@testset "EllipsoidBasics" begin
    ...
    result = newton_method(f, df, ddf, interval=interval, x0=x0, ϵ=ϵ, verbose=false, stopIfNegative=false)
    # Check the result
    @test isapprox(result[1], -3.0, atol=ϵ) && interval[1] ≤ result[2] ≤ interval[2]

    result = newton_method(f, df, ddf, interval=[0,4], x0=x0, ϵ=ϵ, verbose=false, stopIfNegative=false)
